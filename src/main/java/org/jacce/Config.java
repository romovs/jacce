...
public class Config {
//	private static final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(Config.class.getName());
		...
		try {
			...
			for (int s = 0; s < cacheList.getLength(); s++) {
				...
				if (node instanceof Element) {
					...
					if (capacity < -1)
						throw new CacheException("Invalid capacity specified for cache: " + id);
					if (capacity == -1)
						capacity = CacheFactory.DEF_CAPACITY;
					
				    boolean cleanSettings = false;
				    
				    int cleanInterval = 0;			// TODO: default values
				    int ttl = 0; 
				    int idlePeriod = 0;
					...
					if (cleanSettings)
						CacheFactory.getInstance().addCacheTimed(id, desc, cacheClassName, capacity, memorySize, cleanInterval, ttl, idlePeriod);
					else
						CacheFactory.getInstance().addCache(id, desc, cacheClassName, capacity, memorySize);
